...
vi.mock('$lib/fetchRequests', () => ({
	fetchDeleteClassroom: vi.fn()
}));

global.fetch = vi.fn();

describe('POST /delete-classroom', () => {
	it('returns 200 with success message when classroom deletion is successful', async () => {
		const mockCookies = {
			get: vi.fn((name) => (name === 'access_token' ? 'valid_token' : 'refresh_token')),
			set: vi.fn(),
			delete: vi.fn()
		};
		const request = new Request('http://localhost/delete-classroom', {
			method: 'POST',
			body: JSON.stringify({ classroom_id: 3 })
		});

		// Mock successful response
		fetchDeleteClassroom.mockResolvedValueOnce({ ok: true });
		...
		expect(jsonResponse).toEqual({ message: 'Classroom deleted' });
	});

	it('returns 500 with error message when classroom deletion fails', async () => {
		const mockCookies = {
			get: vi.fn((name) => (name === 'access_token' ? 'valid_token' : 'refresh_token')),
			set: vi.fn(),
			delete: vi.fn()
		...
		const request = new Request('http://localhost/delete-classroom', {
			method: 'POST',
			body: JSON.stringify({ classroom_id: 2 })
		});

		fetchDeleteClassroom.mockResolvedValueOnce({ ok: false });

		const response = await POST({ request, cookies: mockCookies });

		expect(response.status).toBe(500);
		const jsonResponse = await response.json();
		expect(jsonResponse).toEqual({ error: 'Failed to delete Classroom' });
	});

	it('refreshes access token and retries classroom deletion successfully', async () => {
		const mockCookies = {
		...
		const request = new Request('http://localhost/delete-classroom', {
			method: 'POST',
			body: JSON.stringify({ classroom_id: 456 })
		});

		// Mock `fetchDeleteClassroom` to return 401 initially, then success
		fetchDeleteClassroom
		...
		expect(response.status).toBe(200);
		expect(jsonResponse).toEqual({ message: 'Classroom deleted' });
		expect(fetchDeleteClassroom).toHaveBeenCalledTimes(2);
	...
	it('redirects to /login if refresh_token is missing', async () => {
		...
		fetchDeleteClassroom.mockResolvedValueOnce({ ok: false, status: 401 });
