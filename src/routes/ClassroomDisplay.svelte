<script lang="ts">
	...
	import { _deleteClassroom } from './+page';
	import { toast } from 'svelte-sonner';

	export let classroom_list: { id: number; title: string }[];
...
<Card.Content class="p-0 mb-6">
	<ul class="grid grid-cols-2 gap-[2rem]">
		{#each classroom_list as classroom (classroom.id)}
			<li>
...
<AlertDialog.Root open={isDialogOpen} on:close={() => (isDialogOpen = false)}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			<AlertDialog.Title
				>Are you sure you want to delete {selected_classroom_title}?</AlertDialog.Title
			>
